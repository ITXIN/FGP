...
#pragma mark -
#pragma mark --- 设置其基本属性
- (void)setupTextSize:(NSInteger)textSize fixedLenght:(NSInteger)fixedLenght numberOfFlips:(NSInteger)numberOfFlips
{
    self.textSize = textSize;
    self.useSound = NO;
    self.fixedLenght = fixedLenght;
    self.flipDuration = 0.1f;
    self.flipDurationRange = 1.0f;
    self.numberOfFlips = numberOfFlips;
    self.numberOfFlipsRange = 1.0f;
...
//    NSInteger len = MAX(self.fixedLenght, text.length);
    NSInteger len = MIN(self.fixedLenght, text.length);
...
- (NSString *)randomAlphabetCharacter {
    static NSString *const alphabet = @"1";
