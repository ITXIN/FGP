...
@property (nonatomic,assign) CGFloat curveW;
//对应哪个红色的点对应的视图
@property (nonatomic, strong) UIButton *curveView;
//性状对应的layer
...
- (CGColorRef)getColor{
    NSInteger r = arc4random()%(33-28+1)+28;
    NSInteger g = arc4random()%(199-167+1)+167;
    return RGBA(r, g, 252,0.3).CGColor;
}
...
//        _shapeLayer.fillColor = RGB(33, 192, 252).CGColor;
         _shapeLayer.fillColor = [self getColor];
        [self.layer addSublayer:_shapeLayer];
        
        // _curveView就是r5点
        _curveX = DEVICE_WIDTH/2.0-25;
        _curveY = MIN_HEIGHT;
        _curveW = ([FGProjectHelper getIsiPad] == YES)? USER_ICON_IPAD_WIDTH:USER_ICON_IPAD_WIDTH/2;
//        _curveView = [[UIButton alloc] initWithFrame:CGRectMake(_curveX, _curveY-10,_curveW, _curveW)];
        _curveView = [UIButton buttonWithType:UIButtonTypeCustom];
        _curveView.frame = CGRectMake(_curveX, _curveY-10,_curveW, _curveW);
//        _curveView.image = [UIImage imageNamed:@"my_center"];
        [_curveView setImage:[UIImage imageNamed:@"my_center"] forState:UIControlStateNormal];
        _curveView.backgroundColor = [UIColor whiteColor];
        _curveView.tag = MyCenterViewActionTypeEditeUserIcon;
        [_curveView addTarget:self action:@selector(myCenterViewAction:) forControlEvents:UIControlEventTouchUpInside];
        _curveView.layer.cornerRadius = _curveW/2;
        _curveView.layer.masksToBounds = YES;
...
- (void)myCenterViewAction:(UIButton*)sender{
    self.myCenterViewActionBlock(sender.tag);
}
                 ...
                 usingSpringWithDamping:0.5
                  initialSpringVelocity:0
                             ...
                             animations:^{
                                 ...
                                 _curveView.frame = CGRectMake(DEVICE_WIDTH/2.0-25, MIN_HEIGHT, CGRectGetWidth(_curveView.frame), CGRectGetHeight(_curveView.frame));
